...
window.setInterval('lastTime()',1000)
  function lastTime(){
     var date1 =new Date()
     ...
     $('.in-day span').html(day)
     $('.in-hours span').html(hour)
     $('.in-min span').html(min)
     $('.in-sec span').html(sec)
}
...
$(document).ready(function(){
    $('.kinder-menu').toggle(function(){
        $(this).addClass('is-active');
        setTimeout(function(){
            $('body').addClass('body-o').animate({'left':'260px'});
            $('.kinderleftbar').animate({'left':'260px'});
            $('.kinderleft').animate({'left':'0px'})
        },500)
    },function(){
        $(this).removeClass('is-active');
        setTimeout(function(){
            $('body').animate({'left':'0px'});
            $('.kinderleftbar').animate({'left':'0px'});
            $('.kinderleft').animate({'left':'-260px'})
            setTimeout(function(){
                $('body').removeClass('body-o');
            },1000)
        },500)
    })

    $('.kinderleftbar ul li a.sina').toggle(function(){
        $('.sina-show').animate({'width':'250px'})
    },function(){
        $('.sina-show').animate({'width':'0px'})
    })
    ...
    $(window).scroll(function(){
        ...
        if($(document).scrollTop() >= 270){
            $('.clock').addClass('clock-on')
        }else{
            $('.clock').removeClass('clock-on')
        }
        if($(this).scrollTop()!=0) {
            $("#toTop").fadeIn(); 
        }else{
